...
class CreateInspectionForm(View):
    ...
    def get(self, request):
        ...
        formset = category_formset(prefix='category',)
    ...
    def post(self, request):
        ...
        if inspection_form.is_valid():
            ...
            instance.save()
            formset = category_formset(request.POST, prefix='category', instance=instance)
            if formset.is_valid():
                formset.save()
            else:
                return render(request, self.template_name, context)
...
def get_options(request):
    options = OptionTitle.objects.get(id=request.GET.get('option_type')).option_title.all().values('label')
    data = {
